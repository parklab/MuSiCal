...
from .utils import indel_types_83_str, snv_types_96_str
...
colorsIndel83.extend(
    [colorPaletteIndel83[12]]
    + [colorPaletteIndel83[13]] * 2
    + [colorPaletteIndel83[14]] * 3
    + [colorPaletteIndel83[15]] * 5
)
...
def sigplot_bar(
    ...
    if sig_type == "SBS":
        if n_features != 96:
            raise ValueError(
                "sig_type is set to SBS but the number of features is not equal to 96."
            )
    elif sig_type == "Indel83":
        if n_features != 83:
            raise ValueError(
                "sig_type is set to Indel83 but the number of features is not equal to 83."
            )
    else:
        pass
    ...
    if colors is None:
        ...
        if type(colors) is str or type(colors) is tuple:
            ...
            if len(colors) != n_features:
                raise ValueError(
                    "If colors is a list, its length must be the same as n_features."
                )
        else:
            raise TypeError(
                "Colors can be a single color, i.e., a str or tuple, or a list of colors."
            )
    ...
    if xticklabels is None:
        ...
        if type(xticklabels) is bool:
            ...
            if len(xticklabels) != n_features:
                raise ValueError(
                    "If xticklabels is a list, its length must be the same as n_features."
                )
        else:
    ...
    if title is None:
        ...
        if type(title) is bool:
            ...
            if len(title) != n_components:
                raise ValueError(
                    "If title is a list, its length must be the same as n_components."
                )
        else:
    ...
    if ylim is not None:
        if type(ylim) is tuple:
            ...
            if len(ylim) != n_components:
                raise ValueError(
                    "When ylim is a list, its length must be the same as n_components."
                )
        else:
    ...
    if fix_size:
        ...
        for sig_index in range(0, n_components):
            ...
            if xticks:
                plt.xticks(
                    x, xticklabels, fontsize=tick_fontsize, rotation=rotation, ha=ha
                )
            else:
                subfig.set_xticks([])
...
def plot_silhouettes(
    ...
    (p1,) = host.plot(
        model.n_components_all,
        sil_score_mean_array,
        color=color1,
    ...
    host.axvspan(
        model.n_components - 0.25, model.n_components + 0.25, color="grey", alpha=0.3
    )
