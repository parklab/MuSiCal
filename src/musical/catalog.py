...
def load_catalog(name="COSMIC-MuSiCal_v3p2_SBS_WGS", sep=",", index_col=0):
    ...
    if name in CATALOG_NAMES:
        catalog = pd.read_csv(
            importlib.resources.open_text(data, name + ".csv"), sep=",", index_col=0
        )
...
class Catalog:
    ...
    def __init__(self, W=None, signatures=None, features=None, name=None):
        if W is None:
            ...
            if signatures is not None:
                warnings.warn(
                    "Columns of W are used as signatures. The provided signatures attribute is ignored.",
                    UserWarning,
                )
            if features is not None:
                warnings.warn(
                    "Index of W is used as features. The provided features attribute is ignored.",
                    UserWarning,
                )
            self._W = W
            self._name = name
    ...
    def restrict_catalog(self, tumor_type=None, is_MMRD=True, is_PPD=True):
        if tumor_type != None:
            ...
            tts_sigs = pd.read_csv(
                importlib.resources.open_text(
                    data, "TumorType_" + self._sig_type + "_Signatures.csv"
                ),
                sep=",",
            )
            ...
            self._signatures = [
                item
                for index, item in enumerate(self._signatures)
                if item in signatures_tt
            ]
        ...
        if not is_MMRD or not is_PPD:
            MMRD_PPD_sigs = pd.read_csv(
                importlib.resources.open_text(
                    data, "MMRD_PPD_" + self._sig_type + "_Signatures.csv"
                ),
                sep=",",
            )
            signatures_MMRD = np.array(
                MMRD_PPD_sigs.loc[MMRD_PPD_sigs["MMRD_PPD_category"] == "MMRD"]
            )
            signatures_MMRD_PPD = np.array(
                MMRD_PPD_sigs.loc[MMRD_PPD_sigs["MMRD_PPD_category"] == "MMRD_PPD"]
            )
            signatures_PPD = np.array(
                MMRD_PPD_sigs.loc[MMRD_PPD_sigs["MMRD_PPD_category"] == "PPD"]
            )
            if not is_MMRD:
                self._signatures = [
                    item
                    for index, item in enumerate(self._signatures)
                    if item not in signatures_MMRD
                ]
                self._signatures = [
                    item
                    for index, item in enumerate(self._signatures)
                    if item not in signatures_MMRD_PPD
                ]
                self._W = self._W[self._signatures]
            if not is_PPD:
                self._signatures = [
    ...
    def normalize_W_catalog(self, sequencing="WES"):
        ...
        if self._sig_type == "SBS":
            weights = pd.read_csv(
                importlib.resources.open_text(data, "TriNucFreq_Weights.csv"),
                sep=",",
                index_col=0,
            )
        else:
