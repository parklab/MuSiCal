...
def _filter_results(X, Ws, Hs, method="error_distribution", thresh=0.05, percentile=90):
    ...
    if len(Ws) == 1:
        ...
        if method == "error_distribution":
            ...
            retained_indices = np.arange(0, len(Ws))[
                np.logical_and(pvalues > thresh, pvalues_tail > thresh)
            ]
            ...
            retained_indices = np.arange(0, len(Ws))[
                ...
                <= thresh * stats.median_abs_deviation(errors)
            ]
            Ws_filtered = [Ws[i] for i in retained_indices]
            Hs_filtered = [Hs[i] for i in retained_indices]
            return Ws_filtered, Hs_filtered, retained_indices
            ...
            retained_indices = np.arange(0, len(Ws))[
                errors <= (thresh + 1.0) * np.min(errors)
            ]
            Ws_filtered = [Ws[i] for i in retained_indices]
            Hs_filtered = [Hs[i] for i in retained_indices]
            return Ws_filtered, Hs_filtered, retained_indices
...
def _gather_results(
    X, Ws, method="cluster_by_matching", metric="cosine", n_components=None
):
    ...
    if method == "hierarchical":
        ...
        if len(set(cluster_membership)) != n_components:
            cluster_membership = (
                sch.cut_tree(linkage, n_clusters=n_components).flatten() + 1
            )
            if len(set(cluster_membership)) != n_components:
                warnings.warn(
                    "Number of clusters output by cut_tree or fcluster is not equal to the specified number of clusters",
                    UserWarning,
                )
        W = []
        n_support = []
        ...
        H = nnls(X, W)
        # Calculate sil_score
        samplewise_sil_score = silhouette_samples(
            d_square_form, cluster_membership, metric="precomputed"
        )
        sil_score = []
        for i in range(0, n_components):
                ...
                if i != j:
                    ...
                    matchings.append(
                        [
                            ...
                            pd.DataFrame(
                                np.array([np.arange(0, n_components), col_ind]).T,
                                columns=[i, j],
                            ),
                        ]
                    )
        ...
        while len(matchings) > 0:
            ...
            if len(result) == 1 and sorted(result[0].columns.values.tolist()) == list(
                range(0, len(Ws))
            ):
                break
            # Processing
        ...
        if len(result) != 1:
            raise RuntimeError(
                "Clustering by matching resulted in a result of length != 1."
            )
        result = result[0]
...
def _select_n_components(
    n_components_all,
    samplewise_reconstruction_errors_all,
    sil_score_all,
    n_replicates,
    n_replicates_after_filtering_all,
    Ws_all=None,
    pthresh=0.05,
    ...
    for n_components in n_components_all:
        if (
            np.mean(sil_score_all[n_components]) >= sil_score_mean_thresh
            and np.min(sil_score_all[n_components]) >= sil_score_min_thresh
            and n_replicates_after_filtering_all[n_components] / n_replicates
            >= n_replicates_filter_ratio_thresh
        ):
    ...
    if method == "consistency":
        ...
        if max_k_all is None:
            max_k_all = {
                n_components: np.max(n_components_all) + 5
                for n_components in n_components_all
            }
        ...
        for n_components in n_components_all:
            if n_components == 1:
                ...
                optimalK = OptimalK(
                    Ws, max_k=max_k_all[n_components], nrefs=nrefs, metric=metric
                )
    ...
    if len(n_components_all) == 1:
        warnings.warn(
            "Only 1 n_components value is tested. Selecting this n_components value.",
            UserWarning,
        )
        return (
            n_components_all[0],
    ...
    pvalue_all = np.array(
        [
            stats.mannwhitneyu(
                samplewise_reconstruction_errors_all[n_components],
                samplewise_reconstruction_errors_all[n_components + 1],
                alternative="greater",
            )[1]
            for n_components in n_components_all[0:-1]
        ]
    )
    ...
    if method == "consistency":
        if len(n_components_consistent) == 0 and len(n_components_stable) == 0:
            ...
            warnings.warn(
                "No consistent n_components values are found and "
            ...
            # This won't usually happen
            warnings.warn(
                "No consistent n_components values are found. "
                "Selecting the greatest n_components with stable solutions.",
                UserWarning,
            )
            n_components_selected = n_components_stable[-1]
        else:
            n_components_intersect = np.array(
                list(set(n_components_consistent).intersection(n_components_stable))
            )
            if len(n_components_intersect) == 0:
                warnings.warn(
                    "Intersection of stable and consistent n_components is empty. "
                    "Selecting the greatest consistent n_components value.",
                    UserWarning,
                )
                n_components_selected = n_components_consistent[-1]
        ...
        for p, p_tail, n_components in zip(
            pvalue_all, pvalue_tail_all, n_components_all[1:]
        ):
        ...
        if len(n_components_stable) == 0 and len(n_components_significant) == 0:
            warnings.warn(
                "No n_components values with stable solutions are found and "
            ...
            n_components_intersect = np.array(
                list(set(n_components_significant).intersection(n_components_stable))
            )
            if len(n_components_intersect) == 0:
                if method == "algorithm1":
                    warnings.warn(
                        "Intersection of stable and significant n_components is empty. "
                        "Selecting the greatest n_components with significant p-values.",
                        UserWarning,
                    )
                    n_components_selected = n_components_significant[-1]
        ...
        for p, p_tail, n_components in zip(
            pvalue_all, pvalue_tail_all, n_components_all[0:-1]
        ):
        ...
        if len(n_components_stable) == 0 and len(n_components_nonsignificant) == 0:
            ...
            n_components_intersect = np.array(
                list(set(n_components_nonsignificant).intersection(n_components_stable))
            )
            if len(n_components_intersect) == 0:
                if method == "algorithm2":
                    warnings.warn(
                        "Intersection of stable and nonsignificant n_components is empty. "
                        "Selecting the smallest n_components with nonsignificant p-values.",
                        UserWarning,
                    )
                    n_components_selected = n_components_nonsignificant[0]
    ...
    if n_components_selected == n_components_all[-1]:
        warnings.warn(
            "Selected n_components is equal to the maximum n_components tested. The optimal n_components might be greater.",
            UserWarning,
        )
    if n_components_selected == n_components_all[0] and n_components_selected > 1:
        warnings.warn(
            "Selected n_components is equal to the minimum n_components tested. The optimal n_components might be smaller.",
            UserWarning,
        )
...
class DenovoSig:
    ...
    def __init__(
        ...
        self.n_components_all = np.arange(
        ...
        self.select_n_replicates_filter_ratio_thresh = (
            select_n_replicates_filter_ratio_thresh
        )
        # mvnmf specific
    ...
    def _run_jobs(self):
        ...
        for n_components in self.n_components_all:
            if self.verbose:
                print(
                    "Extracting signatures for n_components = "
                    + str(n_components)
                    + ".................."
                )
            if self.method == "nmf":
                parameters = [
                    (index_replicate, n_components, None)
                ...
                if self.mvnmf_hyperparameter_method == "all":
                    ...
                    if self.verbose:
                        print(
                            "Selected lambda_tilde = %.3g. This lambda_tilde will be used for all subsequent mvNMF runs."
                            % model.lambda_tilde
                        )
                    # Run the rest of the models, using preselected hyperparameter
                    parameters = [
                        (index_replicate, n_components, lambda_tilde)
                    ...
                    if type(self.mvnmf_lambda_tilde_grid) is not float:
                        raise ValueError(
                            "When mvnmf_hyperparameter_method is set to fixed, a single float value must be provided for mvnmf_lambda_tilde_grid."
                        )
                    parameters = [
                        (index_replicate, n_components, None)
                        for index_replicate in range(0, self.n_replicates)
                    ...
                    workers.close()
                    workers.join()
            self.W_raw_all[n_components] = [
                model.W for model in models
            ...
            if self.method == "nmf":
                ...
                if self.mvnmf_hyperparameter_method == "all":
                    self.lambda_tilde_all[n_components] = [
                        model.lambda_tilde for model in models
                    ]
                elif self.mvnmf_hyperparameter_method == "single":
                    self.lambda_tilde_all[n_components] = [
                        lambda_tilde
    ...
    def postprocess(self):
        ...
        if self.select_method == "consistency":
            if self.cluster_method != "hierarchical":
                warnings.warn(
                    "Select_method is consistency. However, cluster_method is %r. It is better to use cluster_method = hierarchical when select_method is consistency."
                    % self.cluster_method,
                    UserWarning,
                )
        elif self.select_method in [
            "algorithm1",
            "algorithm1.1",
            "algorithm2",
            "algorithm2.1",
        ...
        for n_components in self.n_components_all:
            ...
            # Gather
            W, H, sil_score, sil_score_mean, n_support = _gather_results(
                self.X, Ws, method=self.cluster_method, metric="cosine"
            )
            ...
            self.reconstruction_error_all[n_components] = beta_divergence(
                self.X, W @ H, beta=1, square_root=False
            )
        self.samplewise_reconstruction_errors_all = {
            n_components: _samplewise_error(
                self.X, self.W_all[n_components] @ self.H_all[n_components]
            )
            for n_components in self.n_components_all
        }
        ...
        (
            self.n_components,
            self.optimal_k_all,
            self.n_components_consistent,
            self.n_components_stable,
            self.pvalue_all,
            self.pvalue_tail_all,
        ...
        self.samplewise_reconstruction_errors = (
            self.samplewise_reconstruction_errors_all[self.n_components]
        )
        self.n_support = self.n_support_all[self.n_components]
        self.W_df = pd.DataFrame(
            self.W,
            columns=["Sig" + str(i) for i in range(1, self.n_components + 1)],
            index=self.features,
        )
    ...
    def plot_selection(self, title=None, plot_pvalues=True, outfile=None, figsize=None):
        ...
        sns.set_style("ticks")
        ##### Collect data
        sil_score_mean = np.array(
            [
                self.sil_score_mean_all[n_components]
                for n_components in self.n_components_all
            ]
        )
        mean_sil_score = np.array(
            [
                np.mean(self.sil_score_all[n_components])
                for n_components in self.n_components_all
            ]
        )
        min_sil_score = np.array(
            [
                np.min(self.sil_score_all[n_components])
                for n_components in self.n_components_all
            ]
        )
        reconstruction_error = np.array(
            [
                self.reconstruction_error_all[n_components]
                for n_components in self.n_components_all
            ]
        )
        ...
        if self.optimal_k_all is not None:
            optimal_k = np.array(
                [
                    self.optimal_k_all[n_components]
                    for n_components in self.n_components_all
                ]
            )
        ...
        host.axvspan(
            self.n_components - 0.25, self.n_components + 0.25, color="grey", alpha=0.3
        )
        ...
        ## Generate heatmap
        heatmap = sns.heatmap(
            sil_score_all_df, vmin=0, vmax=1, cmap="YlGnBu", ax=heatmap, square=True
        )
    ...
    def assign(
        ...
        if hasattr(self, "_assign_is_run"):
            warnings.warn(
                "self.assign has been previously called. This call will replace the previous results.",
                UserWarning,
            )
        if hasattr(self, "_assign_grid_is_run"):
            warnings.warn(
                "self.assign_grid has been previously called. Therefore running self.assign may make some attributes inconsistent. Be careful.",
                UserWarning,
            )
        # Check input
        ...
        if np.sum(self.features == W_catalog.index.values) != self.n_features:
            warnings.warn(
                "W_catalog has different feature names. The feature names of W_catalog will be converted to self.features. Make sure that the features match.",
                UserWarning,
            )
        ...
        if self.W.shape[0] == 96:
            if not clean_W_s:
                warnings.warn(
                    "For 96-dimensional SBS signatures, you can try clean_W_s=True.",
                    UserWarning,
                )
        self.clean_W_s = clean_W_s
        self.W_s, self.H_s, self.sig_map = assign(
            self.X_df,
            self.W_df,
            self.W_catalog,
            method=self.method_assign,
            thresh_match=self.thresh_match,
            thresh_refit=self.thresh_refit,
    ...
    def assign_grid(
        ...
        W_catalog,
        method_assign="likelihood_bidirectional",
        thresh_match_grid=None,
        thresh_refit_grid=None,
        thresh_new_sig=0.0,
        connected_sigs=False,
        clean_W_s=False,
    ):
        ...
        if hasattr(self, "_assign_is_run"):
            warnings.warn(
                "self.assign has been previously called. Therefore running self.assign_grid may make some attributes inconsistent. Be careful.",
                UserWarning,
            )
        if hasattr(self, "_assign_grid_is_run"):
            warnings.warn(
                "self.assign_grid has been previously called. This call will replace the previous results.",
                UserWarning,
            )
        # Check input
        if not isinstance(W_catalog, pd.DataFrame):
        ...
        W_s_grid_1d, H_s_grid, sig_map_grid_1d, thresh_match_grid_unique = assign_grid(
            ...
            self.W_catalog,
            method=self.method_assign,
            thresh_match_grid=self.thresh_match_grid,
            thresh_refit_grid=self.thresh_refit_grid,
            thresh_new_sig=self.thresh_new_sig,
            connected_sigs=self.connected_sigs,
            clean_W_s=self.clean_W_s,
            ncpu=self.ncpu,
        ...
        for thresh_match in self.thresh_match_grid:
            for thresh_refit in self.thresh_refit_grid:
                sigs_assigned = (
                    H_s_grid[thresh_match][thresh_refit]
                    .index[H_s_grid[thresh_match][thresh_refit].sum(1) > 0]
                    .values
                )
                self.sigs_assigned_grid[(thresh_match, thresh_refit)] = sigs_assigned
                self.W_s_grid[(thresh_match, thresh_refit)] = pd.DataFrame.copy(
                    W_s_grid_1d[thresh_match][sigs_assigned]
                )
                self.H_s_grid[(thresh_match, thresh_refit)] = pd.DataFrame.copy(
                    H_s_grid[thresh_match][thresh_refit].loc[sigs_assigned]
                )
                self.n_sigs_assigned_grid[(thresh_match, thresh_refit)] = len(
                    sigs_assigned
                )
    ...
    def validate(self, W_s=None, H_s=None, validate_n_replicates=1):
        ...
        if W_s is not None and H_s is not None:
            if hasattr(self, "W_s") or hasattr(self, "H_s"):
                warnings.warn(
                    "Signature assignment W_s or H_s already present in the model. "
            ...
            if H_s.index.tolist() != W_s.columns.tolist():
                raise ValueError(
                    "The provided W_s and H_s do not have matched signatures."
                )
            self.W_s = W_s
            self.H_s = H_s
            ...
            if not hasattr(self, "W_s") or not hasattr(self, "H_s"):
                raise ValueError(
                    "Signature assignment has not been done yet, and external assignment is not provided. "
        ...
        for i in range(0, self.validate_n_replicates):
            # Simulate data
            X_simul = pd.DataFrame(
                simulate_count_matrix(self.W_s.values, self.H_s.values),
            ...
            model_simul.n_components_all = np.array(
                [self.n_components]
            )  # Fix n_components.
            ...
            if self.method == "mvnmf":  # If mvnmf, fix lambda_tilde
                if self.mvnmf_hyperparameter_method == "all":
                    raise ValueError(
                        "De novo discovery is performed with mvNMF and mvnmf_hyperparameter_method is set to all. However, current implementation of validation_grid is not compatible with that."
                    )
                model_simul.mvnmf_hyperparameter_method = "fixed"
                model_simul.mvnmf_lambda_tilde_grid = float(
                    self.lambda_tilde_all[self.n_components][0]
                )
            model_simul.fit()
            # Collect data
            W_simul, col_index, pdist = match_catalog_pair(
                self.W, model_simul.W, metric="cosine"
            )
            H_simul = model_simul.H[col_index, :]
            W_cos_dist = pdist[np.arange(0, self.n_components), col_index].mean()
            H_frobenius_dist = beta_divergence(
                self.H, H_simul, beta=2, square_root=True
            )
            self.X_simul.append(X_simul)
            self.W_simul.append(
                pd.DataFrame(W_simul, columns=self.signatures, index=self.features)
            )
            self.H_simul.append(
                pd.DataFrame(H_simul, columns=self.samples, index=self.signatures)
            )
    ...
    def _select_best_grid_point(self):
        ...
        candidate_grid_points = [
            key
            for key in self.W_simul_grid.keys()
            if len(set(self.sigs_assigned_grid[key]).intersection(self.signatures)) == 0
        ]
        if len(candidate_grid_points) == 0:
            warnings.warn(
                "During grid search, all solutions contain new signatures. This could potentially mean that "
        ...
        if self.grid_selection_method == "distance":
            ...
            W_cos_dist_min = np.min(
                [self.W_cos_dist_mean_grid[key] for key in candidate_grid_points]
            )
            # Candidates: W_cos_dist within W_cos_dist_min + self.grid_selection_cos_thresh
            candidate_grid_points = [
                key
                for key in candidate_grid_points
                if self.W_cos_dist_mean_grid[key]
                <= W_cos_dist_min + self.grid_selection_cos_thresh
            ]
            if len(candidate_grid_points) == 1:
                self.best_grid_point = candidate_grid_points[0]
            else:
                # 3. Look at number of assigned sigs
                n_sigs_assigned_min = np.min(
                    [self.n_sigs_assigned_grid[key] for key in candidate_grid_points]
                )
                candidate_grid_points = [
                    key
                    for key in candidate_grid_points
                    if self.n_sigs_assigned_grid[key] == n_sigs_assigned_min
                ]
                if len(candidate_grid_points) == 1:
                    self.best_grid_point = candidate_grid_points[0]
                else:
                    if self.grid_selection_use_H:
                        # 4. Look at H error
                        tmp = [
                            [key, self.H_frobenius_dist_mean_grid[key]]
                            for key in candidate_grid_points
                        ]
        ...
        ### Looking at p-values
            ...
            for key in candidate_grid_points:
                W_simul = np.mean(
                    self.W_simul_grid[key], 0
                )  # We take the average signatures of multiple replicates of simulation results here.
                error = np.absolute((self.W - W_simul).flatten())
                # elementwise_errors_W.append([key, np.mean(error), error])
                elementwise_errors_W.append(
                    [key, self.W_cos_dist_mean_grid[key], error]
                )  # We can decide how to identify the 'best' one.
            # Best result
            ...
            for key, _, error in elementwise_errors_W:
                ...
                pvalue_tail = differential_tail_test(
                    error_best, error, percentile=95, alternative="less"
                )[1]
                self.W_simul_error_pvalue_grid[key] = pvalue
                self.W_simul_error_pvalue_tail_grid[key] = pvalue_tail
                if (
                    pvalue > self.grid_selection_pvalue_thresh
                    and pvalue_tail > self.grid_selection_pvalue_thresh
                ):
                ...
                if len(candidate_grid_points) == 1:
                    ...
                    if self.grid_selection_use_H:
                        ...
                        for key in candidate_grid_points:
                            ...
                            elementwise_errors_H.append(
                                [key, self.H_frobenius_dist_mean_grid[key], error]
                            )
                        elementwise_errors_H = sorted(
                            elementwise_errors_H, key=itemgetter(1)
                        )
                        ...
                        for key, _, error in elementwise_errors_H:
                            pvalue = stats.mannwhitneyu(
                                error_best, error, alternative="less"
                            )[1]
                            pvalue_tail = differential_tail_test(
                                error_best, error, percentile=95, alternative="less"
                            )[1]
                        ...
                        if len(candidate_grid_points) == 1:
                            ...
                            tmp = [
                                [key, key[0], key[1]] for key in candidate_grid_points
                            ]
        ...
        self.H_frobenius_dist_mean = self.H_frobenius_dist_mean_grid[
            self.best_grid_point
        ]
        return self
    ...
    def validate_grid(
        ...
        grid_selection_use_H=False,
        grid_selection_pvalue_thresh=0.05,
        grid_selection_cos_thresh=0.02,
    ):
        ...
        for thresh_match in self.thresh_match_grid_unique:
            for thresh_refit in self.thresh_refit_grid:
                ...
                for i in range(0, self.validate_n_replicates):
                    # Simulate data
                    _X_simul = pd.DataFrame(
                        simulate_count_matrix(
                            self.W_s_grid[(thresh_match, thresh_refit)].values,
                            self.H_s_grid[(thresh_match, thresh_refit)].values,
                        ),
                        columns=self.samples,
                        index=self.features,
                    )
                    ...
                    model_simul.fit()
                    # Collect data
                    _W_simul, col_index, pdist = match_catalog_pair(
                        self.W, model_simul.W, metric="cosine"
                    )
                    _H_simul = model_simul.H[col_index, :]
                    _W_cos_dist = pdist[
                        np.arange(0, self.n_components), col_index
                    ].mean()
                    _H_frobenius_dist = beta_divergence(
                        self.H, _H_simul, beta=2, square_root=True
                    )
                    X_simul.append(_X_simul)
                    W_simul.append(
                        pd.DataFrame(
                            _W_simul, columns=self.signatures, index=self.features
                        )
                    )
                    H_simul.append(
                        pd.DataFrame(
                            _H_simul, columns=self.samples, index=self.signatures
                        )
                    )
                ...
                self.H_frobenius_dist_grid[(thresh_match, thresh_refit)] = (
                    H_frobenius_dist
                )
                self.W_cos_dist_mean_grid[(thresh_match, thresh_refit)] = np.mean(
                    W_cos_dist
                )
                self.H_frobenius_dist_mean_grid[(thresh_match, thresh_refit)] = np.mean(
                    H_frobenius_dist
                )
        ################# Select best result
