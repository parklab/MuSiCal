...
def nnls_thresh_naive(
    x, W, thresh=0.05, thresh_agnostic=0.0, indices_associated_sigs=None
):
    ...
    if thresh_agnostic > 0:
        ...
        indices_retained = indices_all[
            (h_normalized >= thresh) & (h >= thresh_agnostic / W_max)
        ]
    else:
        h, _ = sp.optimize.nnls(W, x)
        h_normalized = h / np.sum(h)
...
def nnls_thresh(x, W, thresh=0.05, thresh_agnostic=0.0, indices_associated_sigs=None):
    ...
    if thresh_agnostic > 0:
        ...
        while True:
            h, _ = sp.optimize.nnls(W[:, indices_retained], x)
            h_normalized = h / np.sum(h)
            indices_retained_next = indices_retained[
                (h_normalized >= thresh)
                & (h >= thresh_agnostic / W_max[indices_retained])
            ]
...
def nnls_likelihood_backward(
    x, W, thresh=0.001, per_trial=True, indices_associated_sigs=None
):
    ...
    while True:
        if len(indices_retained) == 1:
            ...
            loglikelihood_current = _multinomial_loglikelihood(
                x, p_current, per_trial=per_trial
            )
            loglikelihoods = []
            ...
            for index in indices_retained:
                ...
                loglikelihoods.append(
                    _multinomial_loglikelihood(x, p, per_trial=per_trial)
                )
            ...
            if np.min(loglikelihoods) >= thresh:
                ...
                indices_retained = np.array(
                    [i for i in indices_retained if i != index_remove]
                )
...
def nnls_likelihood_bidirectional(
    x,
    W,
    ...
    if thresh_backward > thresh_forward:
        warnings.warn(
            "thresh_backward is greater thresh_forward. This might lead to indefinite loops.",
            UserWarning,
        )
    n_sigs = W.shape[1]
    indices_all = np.arange(0, n_sigs)
    ...
    while i_iter < max_iter:
        ...
        if len(indices_retained) == 1:
            ...
            if np.min(loglikelihoods) >= thresh_backward:
                ...
                # print(np.min(loglikelihoods))
        ########################## Forward ##########################
        indices_others = np.array(
            sorted(list(set(indices_all) - set(indices_retained)))
        )
        ...
        if not backward_stop and not forward_stop and index_remove == index_add:
            warnings.warn(
                "The same signature is being removed and added back within one iteration, suggesting ill convergence.",
                UserWarning,
            )
            break
    if i_iter >= max_iter:
        warnings.warn(
            "Max_iter reached, suggesting that the problem may not converge. Or try increasing max_iter.",
            UserWarning,
        )
    ### Final NNLS
    if indices_associated_sigs is not None:
...
def nnls_cosine_bidirectional(
    x,
    W,
    thresh_backward=0.01,
    thresh_forward=None,
    max_iter=1000,
    indices_associated_sigs=None,
):
...
def nnls_likelihood_backward_relaxed(
    x, W, thresh=0.001, per_trial=True, indices_associated_sigs=None
):
    n_sigs = W.shape[1]
    indices_all = np.arange(0, n_sigs)
    ...
    while True:
        if len(indices_retained) == 1:
            ...
            for index in indices_retained:
                _indices = np.array(
                    [i for i in indices_all if i != index]
                )  # !!!!!! Difference in here.
                p = W[:, _indices] @ sp.optimize.nnls(W[:, _indices], x)[0]
                p = p / np.sum(p)
                loglikelihoods.append(
...
def nnls_likelihood_bidirectional_relaxed(
    x,
    W,
    thresh_backward=0.001,
    thresh_forward=None,
    ...
    if thresh_backward > thresh_forward:
        warnings.warn(
            "thresh_backward is greater than thresh_forward. This might lead to indefinite loops.",
            UserWarning,
        )
    n_sigs = W.shape[1]
    indices_all = np.arange(0, n_sigs)
...
class SparseNNLS:
    ...
    def fit(self, X, W):
        ...
        if isinstance(W, pd.DataFrame):
            ...
            self.W = pd.DataFrame(
                W, columns=["Sig" + str(i) for i in range(0, W.shape[1])]
            )
            self.sigs_all = self.W.columns.values
        if isinstance(X, pd.DataFrame):
            ...
            if len(X.shape) == 1:
                self.X = pd.DataFrame(
                    np.reshape(X, (-1, 1)),
                    columns=["Sample" + str(i) for i in range(0, 1)],
                    index=self.W.index,
                )
            else:
                self.X = pd.DataFrame(
                    X,
                    columns=["Sample" + str(i) for i in range(0, X.shape[1])],
                    index=self.W.index,
                )
            self.samples = self.X.columns.values
        ##########
        ...
        if self.method == "thresh_naive":
            ...
            self.H = [
                nnls_thresh_naive(
                    ...
                    self.W.values,
                    thresh=self.thresh,
                    thresh_agnostic=self.thresh_agnostic,
                    indices_associated_sigs=self.indices_associated_sigs,
                )
                for x in self._X_in.T.values
            ]
            ...
            if self.thresh2 is not None:
                warnings.warn(
                    "Method is chosen as likelihood_backward. The supplied thresh2 will not be used and thus is set to None.",
                    UserWarning,
                )
                self.thresh2 = None
            self.H = [
                nnls_likelihood_backward(
                    x,
                    self.W.values,
                    thresh=self.thresh,
            ...
            if self.thresh2 is not None:
                warnings.warn(
                    "Method is chosen as likelihood_backward_relaxed. The supplied thresh2 will not be used and thus is set to None.",
                    UserWarning,
                )
                self.thresh2 = None
            self.H = [
                nnls_likelihood_backward_relaxed(
                    x,
                    self.W.values,
                    thresh=self.thresh,
                    per_trial=self.per_trial,
            ...
            if self.per_trial is None:
                self.per_trial = True
            self.H = [
                nnls_likelihood_bidirectional(
                    x,
                    self.W.values,
                    thresh_backward=self.thresh_backward,
                    thresh_forward=self.thresh_forward,
            ...
            if self.per_trial is None:
                self.per_trial = True
            self.H = [
                nnls_likelihood_bidirectional_relaxed(
                    x,
                    self.W.values,
                    thresh_backward=self.thresh_backward,
                    thresh_forward=self.thresh_forward,
            ...
            for x, h in zip(self.X.T.values, self.H):
                ...
                if self.indices_associated_sigs is not None:
                    for ind_pair in self.indices_associated_sigs:
                        if any(item in ind_pair for item in indices_retained):
                            indices_retained = np.unique(
                                np.append(indices_retained, ind_pair)
                            )
                ###
        ...
        # Collect final data
        self.H = pd.DataFrame(
            np.array(self.H).T, index=self.W.columns, columns=self.X.columns
        )
        self.X_reconstructed = pd.DataFrame(
            np.array([self.W.values @ h for h in self.H.T.values]).T,
        ...
        self.matched_sigs = []
        self.coefficients = []
        for x, h, x_reconstructed in zip(
            self.X.T.values, self.H.T.values, self.X_reconstructed.T.values
        ):
            self.matched_sigs.append(self.sigs_all[h > 0])
            self.coefficients.append(h[h > 0])
            self.cos_similarities.append(
                1 - sp.spatial.distance.cosine(x, x_reconstructed)
            )
        ...
        self.H_reduced_normalized = pd.DataFrame(
            normalize(self.H_reduced.values, norm="l1", axis=0),
            index=self.H_reduced.index,
            columns=self.H_reduced.columns,
        )
...
class SparseNNLSGrid:
    ...
    def fit(self, X, W):
        ...
        if self.method == "thresh_naive":
            ...
            if self.thresh1_grid is None:
                self.thresh1_grid = np.concatenate(
                    [np.array([0.0]), np.geomspace(0.0001, 5, 50)]
                )
            if self.thresh2_grid is None:
        ...
        if self.ncpu == 1:
            ...
            for thresh1 in self.thresh1_grid:
                for thresh2 in self.thresh2_grid:
                    ...
                    self.cos_similarities_grid[(thresh1, thresh2)] = (
                        model.cos_similarities
                    )
        else:
            parameters = []
            for thresh1 in self.thresh1_grid:
