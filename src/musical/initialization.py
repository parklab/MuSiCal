...
def _init_cluster(
    X, n_components, metric="cosine", linkage="average", max_ncluster=100, min_nsample=1
):
    ...
    for ncluster in range(n_components, np.min([n_samples, max_ncluster]) + 1):
        ...
        if len(set(cluster_membership)) != ncluster:
            cluster_membership = (
                sch.cut_tree(linkage, n_clusters=ncluster).flatten() + 1
            )
            if len(set(cluster_membership)) != ncluster:
                warnings.warn(
...
def initialize_nmf(
    ...
    n_components,
    init="random",
    init_normalize_W=None,
    init_refit_H=None,
    init_cluster_metric="cosine",
    init_cluster_linkage="average",
    init_cluster_max_ncluster=100,
    init_cluster_min_nsample=1,
    init_W_custom=None,
    init_H_custom=None,
):
    ...
    if init_normalize_W is None:
        ...
        if init_normalize_W and init == "custom":
            warnings.warn(
                "init_normalize_W is set to True and init is custom. This might change init_W_custom.",
                UserWarning,
            )
        if (not init_normalize_W) and init in ["spa"]:
            warnings.warn(
                "init is %r but init_normalize_W is set to False. Normalizing W is recommended."
                % init,
                UserWarning,
            )
        if (not init_normalize_W) and init == "cluster":
            warnings.warn(
                "init is cluster and init_normalize_W is set to False. W will be normalized regardless of init_normalize_W when init is cluster.",
                UserWarning,
            )
            init_normalize_W = True
    else:
    ...
    if init_refit_H is None:
        ...
        if init_refit_H and init == "custom":
            warnings.warn(
                "init_refit_H is set to True and init is custom. This might change init_H_custom.",
                UserWarning,
            )
        if init_refit_H and init in ["random", "nndsvd", "nndsvda", "nndsvdar"]:
            warnings.warn(
                "init_refit_H is set to True and init is %r. This might not be intended."
                % init,
                UserWarning,
            )
        if (not init_refit_H) and init in ["cluster", "spa"]:
            warnings.warn(
                "init_refit_H is set to False and init is %r. H will be calculated by NNLS refitting anyway."
                % init,
                UserWarning,
            )
            init_refit_H = True
    ...
    if init == "custom":
        if init_W_custom is None or init_H_custom is None:
            raise ValueError(
                "init is set to custom. Must provide init_W_custom and init_H_custom."
            )
        else:
    ...
    if init == "random":
        ...
        W, H, _ = _init_spa(
            X, n_components, normalize_X=True, normalize_W=init_normalize_W
        )
        return W, H
    elif init == "custom":
        W = init_W_custom
