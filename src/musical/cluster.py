...
class OptimalK:
    ...
    def _simulate_reference_data(self, method="a"):
        ...
        if method == "a":
            ...
            self.reference_data = [
                np.random.random_sample(size=(self.n_features, self.n_samples))
                * (b - a)
                + a
                for i in range(0, self.nrefs)
            ]
            ...
            self.reference_data = [
                (
                    ((np.random.random_sample(size=X.shape) * (b - a) + a) @ VT)
                    + offset
                ).T
                for i in range(0, self.nrefs)
            ]
            ...
            raise ValueError(
                "Method for _simulate_reference_data can only be a or b currently."
            )
    ...
    def _cluster_statistic(X, max_k, metric="cosine", linkage_method="average"):
        ...
        for k in range(1, max_k + 1):
            ...
            if k == 1:
                ...
                silscore_per_sample = silhouette_samples(
                    d_square_form, cluster_membership, metric="precomputed"
                )
                silscorek.append(np.mean(silscore_per_sample))
                silscorek_percluster[k] = np.array(
                    [
                        np.mean(silscore_per_sample[cluster_membership == i])
    ...
    def select(self):
        ...
        self.Wk, self.silscorek, self.silscorek_percluster = self._cluster_statistic(
            self.X, self.max_k, metric=self.metric, linkage_method=self.linkage_method
        )
        ...
        for data in self.reference_data:
            Wk, silscorek, silscorek_percluster = self._cluster_statistic(
                data, self.max_k, metric=self.metric, linkage_method=self.linkage_method
            )
        ...
        self.silscorek_ref_sd = np.std(self.silscorek_ref_all, axis=0) * np.sqrt(
            1 + 1 / self.nrefs
        )
        self.Wk_log_ref_sd = np.std(self.Wk_log_ref_all, axis=0) * np.sqrt(
            1 + 1 / self.nrefs
        )
        ...
        ### Selecting optimal k
        # Using Wk
        candidates = self.ks[1:][
            (self.gap_statistic[1:] - self.Wk_ref_sd[1:] - self.gap_statistic[0:-1])
            <= 0
        ]
        self.k_gap_statistic_valid = candidates - 1
        if len(candidates) == 0:
            warnings.warn(
                "Based on the gap statistic, no k value is a valid candidate. The greatest k is chosen.",
                UserWarning,
            )
        ...
        candidates = self.ks[1:][
            (
                self.gap_statistic_log[1:]
                - self.Wk_log_ref_sd[1:]
                - self.gap_statistic_log[0:-1]
            )
            <= 0
        ]
        ...
        if len(candidates) == 0:
            warnings.warn(
                "Based on the log gap statistic, no k value is a valid candidate. The greatest k is chosen.",
                UserWarning,
            )
        ...
        if np.sum(np.isnan(self.silscorek[1:])) == len(self.silscorek[1:]):
            warnings.warn(
                "All silhouette scores are nan. The greatest k is chosen.", UserWarning
            )
        ...
        self.summary = pd.DataFrame(
            {
                ...
                "diff": list(
                    self.gap_statistic[0:-1]
                    - self.gap_statistic[1:]
                    + self.Wk_ref_sd[1:]
                )
                + [np.nan],
                "diff_log": list(
                ...
                "k_valid_gap_log": [
                    k in self.k_gap_statistic_log_valid for k in self.ks
                ],
            }
        )
        self.summary = self.summary.set_index("k")
        ### Finally cluster according to the optimal k
        _, self.cluster_membership = hierarchical_cluster(
            self.X, self.k, metric=self.metric, linkage_method=self.linkage_method
        )
    ...
    def plot(self, sil_thresh=None, outfile=None, figsize=(10, 10), main_only=True):
        ...
        subfig.errorbar(
            self.summary.index,
            self.summary["gap"],
            yerr=self.summary["sk"],
            fmt=".--",
            markersize=10,
            capsize=3,
            capthick=2,
            color=colorPaletteMathematica97[0],
            label="All k's",
            zorder=0,
        )
        subfig.plot(
            self.summary[self.summary["k_valid_gap"]].index,
            self.summary[self.summary["k_valid_gap"]]["gap"],
            ".",
            markersize=10,
            color=colorPaletteMathematica97[2],
            label="Reasonable k's",
            zorder=1,
        )
        subfig.axvspan(
            self.k_gap_statistic - 0.25,
            self.k_gap_statistic + 0.25,
            color="grey",
            alpha=0.3,
            label="Selected k",
        ...
        plt.legend(
            loc="center left", bbox_to_anchor=(1, 0.5), ncol=1, prop={"size": 14}
        )
        ...
        if not main_only:
            ...
            subfig.errorbar(
                self.summary.index,
                self.summary["gap_log"],
                yerr=self.summary["sk_log"],
                fmt=".--",
                markersize=10,
                capsize=3,
                capthick=2,
            ...
            subfig.plot(
                self.summary[self.summary["k_valid_gap_log"]].index,
                self.summary[self.summary["k_valid_gap_log"]]["gap_log"],
                ".",
                markersize=10,
                color=colorPaletteMathematica97[2],
                label="Reasonable k's",
            ...
            subfig.errorbar(
                self.summary.index,
                self.summary["sil_score_ref"],
                yerr=self.summary["sil_score_ref_std"],
                fmt="--",
                markersize=10,
                capsize=3,
                capthick=2,
                label="Reference data",
                color=colorPaletteMathematica97[1],
                zorder=2,
            )
            for k in self.ks:
            ...
            if type(sil_thresh) is float:
                subfig.axhline(
                    y=sil_thresh, linestyle="--", color="red", alpha=0.5, zorder=0
                )
            plt.legend(
                loc="center left", bbox_to_anchor=(1, 0.5), ncol=1, prop={"size": 14}
            )
