...
def refit(X, W, method="likelihood_bidirectional", thresh=None, connected_sigs=False):
    ...
    # SparseNNLS
    if connected_sigs:
        indices_associated_sigs, _ = get_sig_indices_associated(
            W.columns.values, W.columns.values
        )
        ...
        if len(missing_sigs) > 0:
            warnings.warn(
                (
                    "In refit: connected_sigs is set to True. The input W contains signatures with connected signatures. "
                    + "However, W is missing some connected signatures. Specifically, W is missing: "
                    + ",".join(missing_sigs)
                    + ". Please fill in these missing sigs in W or make sure this is indeed what is wanted."
                ),
                UserWarning,
            )
    else:
    ...
    model = SparseNNLS(
        method=method, thresh1=thresh, indices_associated_sigs=indices_associated_sigs
    )
    model.fit(X, W)
...
def refit_grid(
    ...
    if connected_sigs:
        ...
        indices_associated_sigs = None
    model = SparseNNLSGrid(
        method=method,
        thresh1_grid=thresh_grid,
        ncpu=ncpu,
        verbose=verbose,
...
def _get_W_s(W, W_catalog, H_reduced, cos_similarities, thresh_new_sig):
    ...
    inds_not_new_sig = np.arange(0, W.shape[1])[
        np.array(cos_similarities) >= thresh_new_sig
    ]
    if len(inds_new_sig) > 0 and len(inds_not_new_sig) > 0:
        ...
        sig_map = pd.DataFrame(
            np.identity(len(inds_new_sig)), columns=W_s.columns, index=W_s.columns
        )
        H_tmp = H_reduced.iloc[:, inds_not_new_sig]
        W_s = pd.concat([W_s, W_catalog[H_tmp.index[H_tmp.sum(1) > 0]]], axis=1)
        sig_map = pd.concat([sig_map, H_tmp.loc[H_tmp.index[H_tmp.sum(1) > 0]]]).fillna(
            0.0
        )
...
def _clean_W_s(W, W_s, sig_map, min_sum_0p01=0.15, min_sig_contrib_ratio=0.25):
    """
    TODO:
    ...
    for i in range(sig_map.shape[1]):
        ...
        for j in range(sig_map.shape[0]):
            ...
            if fij < 1 and fij != 0:
                ...
                if (
                    sum_above_0p01 < min_sum_0p01
                    and max_sig_contrib_ratio < min_sig_contrib_ratio
                ):
                    set_to_zero = True
                    sig_map.iat[j, i] = 0
        if set_to_zero:
            weights, _ = sp.optimize.nnls(
                np.array(
                    W_s.loc[:, sig_map.index[sig_map.loc[:, sig_map.columns[i]] > 0]]
                ),
                np.array(W.loc[:, sig_map.columns[i]]),
            )
            sig_map.loc[sig_map.loc[:, sig_map.columns[i]] > 0, sig_map.columns[i]] = (
                weights
            )
...
def match(
    ...
    if len(set(W.columns).intersection(W_catalog.columns)) > 0:
        raise ValueError(
            "W and W_catalog cannot contain signatures with the same name."
        )
    if W.shape[0] == 96:
        if not clean_W_s:
            warnings.warn(
    ...
    W_s, sig_map = _get_W_s(
        W, W_catalog, model.H_reduced, model.cos_similarities, thresh_new_sig
    )
    if clean_W_s:
        W_s, sig_map = _clean_W_s(W, W_s, sig_map)
...
def match_grid(
    ...
    for thresh in thresh_grid:
        ...
        W_s, sig_map = _get_W_s(
            W,
            W_catalog,
            model.H_reduced_grid[key],
            model.cos_similarities_grid[key],
            thresh_new_sig,
        )
        if clean_W_s:
            W_s, sig_map = _clean_W_s(W, W_s, sig_map)
...
def assign(
    ...
    H_s, _ = refit(
        X, W_s, method=method, thresh=thresh_refit, connected_sigs=connected_sigs
    )
...
def assign_grid(
    ...
    W_s_grid_1d, sig_map_grid_1d, _ = match_grid(
        W,
        W_catalog,
        thresh_new_sig=thresh_new_sig,
        method=method,
        thresh_grid=thresh_match_grid,
        ncpu=ncpu,
        verbose=verbose,
        connected_sigs=connected_sigs,
        clean_W_s=clean_W_s,
    ...
    for thresh_match in thresh_match_grid:
        ...
        if sigs in calculated_matching_results.keys():
            ...
            H_s_grid_1d, _ = refit_grid(
                X,
                W_s_grid_1d[thresh_match],
                method=method,
                thresh_grid=thresh_refit_grid,
                ncpu=ncpu,
                verbose=verbose,
                connected_sigs=connected_sigs,
            )
