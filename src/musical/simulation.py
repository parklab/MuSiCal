...
def simulate_LDA(
    ...
    if type(alpha) is float:
        ...
        if len(alpha) != n_components:
            raise ValueError(
                "When alpha is a np.ndarray, it should be of length n_components."
            )
    ...
    if type(n_mutations) is int or type(n_mutations) is float:
        ...
        if len(n_mutations) != n_samples:
            raise ValueError(
                "When n_mutations is a numpy.ndarray, its length must be n_samples."
            )
    else:
    ...
    if W is not None:
        ...
        if W.shape[1] != n_components:
            raise ValueError(
                "The number of signatures in the provided W is different from n_components."
            )
        if W.shape[0] != n_features:
            raise ValueError(
                "The number of features in the provided W is different from n_features."
            )
        if beta is not None:
            warnings.warn(
                "Both W and beta are provided. W will be used and beta will be ignored.",
                UserWarning,
            )
    else:
        ...
        if beta is None:
            ...
            if len(beta) != n_components:
                raise ValueError(
                    "When beta is a np.ndarray, it should be of length n_components."
                )
